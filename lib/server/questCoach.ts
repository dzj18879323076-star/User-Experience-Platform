...
const reportCaseKnowledgeBase = `
【产品体验报告案例抽象】
参考新人体验报告 case，合格报告不是固定模板，但通常具备以下质量：
1. 开篇先交代基本信息：目标产品、体验版本/设备/时间、体验用户画像、使用场景与评价/看评习惯，让读者知道结论从什么人和什么场景出发。
2. 体验目标与思路要明确：说明为什么体验、用什么分析框架、覆盖哪些链路；案例中常见链路包括种草、主动搜索找店、团购/榜单、POI 页、写评、评分、商家端评分诊断。
3. 体验总结先给可讨论结论，再展开细节；好的总结会按触发/能力/动机、看评场景、商家端/用户端、链路模块等维度，直接说明现状、不足、竞品差异和机会点。
4. 体验详情要按模块沉淀：每个模块都要写清场景、页面/入口、体验现象、为什么影响用户/商家/平台、可借鉴竞品、建议和待验证指标。
5. 表达方式要产品化：少写“我觉得不好”，多写“入口深导致触发弱”“评分区分度不足降低找好店效率”“评价样本好评导向影响真实感”“商家端诊断信息不可行动”。
6. 输出要有边界：没有样本/截图/路径/竞品/数据口径时，要标注为待验证问题；不能为了完整而编造结论。
`;
...
本报告基于「抖音评价评分-用户体验平台」中的真实体验记录，围绕看评消费、评价质量、商品/地点评价、创作者链路、商家价值和平台分发进行整理。当前草稿只使用已记录材料，缺失部分保留为待验证问题。

## 二、体验目标与分析思路

- 目标：从新人视角快速理解评价评分在消费者决策、商家经营反馈、作者内容供给和平台分发治理中的作用。
- 思路：按真实路径拆解场景，先记录入口、页面现象和评价样本，再判断它影响什么决策，最后沉淀产品机会和待验证指标。
- 边界：未记录截图、样本、竞品或数据口径的内容只作为待验证问题，不包装为确定结论。
...
硬性要求：
- 必须使用上方【评价评分业务知识库】、【产品体验报告案例抽象】和【对话式闯关规则】进行判断、追问和报告组织。
...
- post_submit_review 的 reportMarkdown 是阶段小结，必须服务最终体验报告；结构至少包括：本关定位、当前证据、初步产品判断、缺失证据、下一步验证。
