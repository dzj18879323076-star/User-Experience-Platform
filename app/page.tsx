...
function buildExportResultMessage(label: string, state: ExportDocResponse) {
  const permissionText = state.permissionHint ? `\n权限说明：${state.permissionHint}` : "";

  if (state.status === "ready" && state.url) {
    return `${label}已生成，并已创建飞书文档：${state.url}${permissionText}`;
  }

  return `${label}已生成，但飞书文档创建暂时走降级：${state.message}${permissionText}\n\n你可以先保留页面里的 Markdown 草稿，等部署环境具备 lark-cli 后再一键创建飞书文档。`;
}
...
export default function QuestPage() {
  ...
  const activeLevelCompleted = isComplete(activeLevel, activeSubmission);
  const canAdvance = activeLevelCompleted;
  ...
  const stageSummaryReadiness = useMemo(
    () => getStageSummaryReadiness(activeLevel, activeSubmission),
    [activeLevel, activeSubmission]
  );
  const finalReportReadiness = useMemo(() => getFinalReportReadiness(state), [state]);
  const stageSummaryDisabled = coachState.status === "loading" || exportDocState.status === "creating" || !stageSummaryReadiness.ready;
  const finalReportDisabled = coachState.status === "loading" || exportDocState.status === "creating" || !finalReportReadiness.ready;
  const filledCurrentCount = currentEntries.length;
  const totalCurrentFields = getLevelFields(activeLevel).length;
  const unlockHint = canAdvance
    ? nextLevel
      ? `本关已通关并发放奖励，可以进入 ${nextLevel.id}。`
      : "全部关卡已通关。"
    : `进入${nextLevel ? ` ${nextLevel.id}` : "下一关"}需要先完成本关核心素材；当前 ${filledCurrentCount}/${totalCurrentFields} 项。优先补真实入口、路径节点、评价样本、决策影响和产品机会。`;
  ...
  async function runGuideAction(actionLabel: string, actionPrompt: string) {
    if (coachState.status === "loading") return;

    const actionQuestion = `[系统操作：${actionLabel}] ${actionPrompt}`;
  ...
  async function createFeishuDocument(markdown: string, title: string) {
    ...
    const nextState: ExportDocResponse = {
      ...
      permissionHint: payload.permissionHint
    };

    setExportDocState({
      status: nextState.status,
      message: nextState.permissionHint ? `${nextState.message} ${nextState.permissionHint}` : nextState.message,
      url: nextState.url
    });
  ...
  function continueAfterReward(reward: LevelReward) {
    const rewardLevelIndex = levels.findIndex((level) => level.id === reward.levelId);
    const unlockedNextLevel = levels[rewardLevelIndex + 1];

    setActiveReward(null);

    if (unlockedNextLevel) {
      switchLevel(unlockedNextLevel.id);
    }
  }
      ...
      <main className="conversation-workspace">
        <nav className="level-map quest-map-panel" aria-label="体验闯关地图">
          ...
          <div className="map-stage" aria-label="生活服务评价体验地图">
            ...
            {levels.map((level, index) => {
              ...
              const previousLevelsComplete = levels.slice(0, index).every((item) => isComplete(item, state.submissions[item.id]));
              const unlocked = active || completed || previousLevelsComplete;
              const locked = !unlocked;
              const progressLabel = completed ? "CLEAR" : active ? "NOW" : unlocked ? "OPEN" : "LOCKED";
              ...
              return (
                <button
                  ...
                  aria-label={`${level.id} ${level.name}：${active ? "当前关卡" : completed ? "已完成，可回看" : unlocked ? `已解锁，已填写 ${filledCount}/${totalCount}` : `未解锁，已填写 ${filledCount}/${totalCount}`}`}
                >
          ...
          <section className="coach-console">
              ...
              <div className="guide-messages" aria-live="polite">
                {coachMessages.slice(-8).map((message) => (
                  <div className={`guide-message ${message.sender}`} key={message.id}>
                    <span>{message.sender === "user" ? "你" : message.sender === "action" ? "系统动作" : coachState.roleName}</span>
                    <p>{message.text}</p>
            ...
            <div className="conversation-actions">
              <button
                ...
                type="button"
                onClick={() => void runGuideAction("追问缺失证据", "请基于当前已沉淀素材指出缺失证据，并把下一步体验动作说清楚。")}
                disabled={coachState.status === "loading" || exportDocState.status === "creating"}
              >
                追问缺失证据
              </button>
              <button className="ghost-button" type="button" onClick={() => void generateStageSummaryDocument()} disabled={stageSummaryDisabled}>
                {exportDocState.status === "creating" ? "生成中..." : stageSummaryReadiness.ready ? "生成阶段小结" : "补齐后生成小结"}
              </button>
              <button className="primary-button" type="button" onClick={() => void generateFullReportDocument()} disabled={finalReportDisabled}>
                {exportDocState.status === "creating" ? "导出中..." : finalReportReadiness.ready ? "生成体验报告" : "补齐后生成报告"}
              </button>
            </div>
            <div className="readiness-panel" aria-label="生成前检查">
              <div className="readiness-card">
                <span>本关素材</span>
                <strong>{filledCurrentCount}/{totalCurrentFields} 个字段已沉淀</strong>
                <p>{stageSummaryReadiness.ready ? "已满足阶段小结生成条件。" : "阶段小结还缺："}</p>
                {!stageSummaryReadiness.ready ? (
                  <ul className="readiness-list">
                    {stageSummaryReadiness.gaps.slice(0, 3).map((gap) => (
                      <li key={gap}>{gap}</li>
                    ))}
                  </ul>
                ) : null}
              ...
              <div className="readiness-card">
                <span>完整报告</span>
                <strong>{completedCount}/{levels.length} 个关卡达到完成线</strong>
                <p>{finalReportReadiness.ready ? "已满足完整报告生成条件。" : "完整报告还缺："}</p>
                {!finalReportReadiness.ready ? (
                  <ul className="readiness-list">
                    {finalReportReadiness.gaps.slice(0, 3).map((gap) => (
                      <li key={gap}>{gap}</li>
                    ))}
                  </ul>
                ) : null}
        ...
        <aside className="side-artifacts" aria-label="报告过程副产物">
          <section className="artifact-card command-card">
            ...
            <p className={`unlock-hint ${canAdvance ? "ready" : "blocked"}`}>{unlockHint}</p>
          </section>
        ...
        <div className="reward-backdrop" role="dialog" aria-modal="true" aria-label="闯关奖励已解锁">
          <div className="reward-modal">
            ...
            <div className="reward-actions">
              ...
              <button className="secondary-button" type="button" onClick={() => continueAfterReward(activeReward)}>
                已固定，继续闯关
              </button>
